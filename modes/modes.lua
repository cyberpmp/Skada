...
-- `extraField` names a second amount drawn as a dimmer continuation of the
-- bar beyond the value (overheal after effective healing), so the bar reads
-- effective + overheal = total cast healing at a glance; the tooltip still
-- carries the numbers.
addMode({ key = "healing", title = "Healing", field = "effectiveHealing", detail = "healingSpells", detailValueField = "effectiveHealing", extraField = "overhealing" })
addMode({ key = "hps", title = "HPS", field = "effectiveHealing", detail = "healingSpells", detailValueField = "effectiveHealing", rate = true, extraField = "overhealing" })
addMode({ key = "overhealing", title = "Overhealing", field = "overhealing", detail = "healingSpells", detailValueField = "overhealing" })
addMode({ key = "healedTargets", title = "Healing Targets", field = "effectiveHealing", detail = "healTargets", detailValueField = "effectiveHealing", extraField = "overhealing" })
...
-- The bar's dimmer continuation (see extraField), in the same units as the
-- value: per-second for rate modes. nil when the mode has none.
function Modes:GetActorExtra(mode, actor)
  if mode.live or not mode.extraField then return nil end
  local extra = actor[mode.extraField] or 0
  if mode.rate then extra = extra / max(1, actor.activeTime or 0) end
  return extra
end

function Modes:GetDetailExtra(mode, spell, actor)
  if not mode.extraField then return nil end
  local extra = spell[mode.extraField] or 0
  if mode.rate then extra = extra / max(1, actor and actor.activeTime or 0) end
  return extra
end
