...
local EXTRA_ALPHA = 0.38

-- Places the continuation from the bar's (eased) fill end to fill end +
-- extra, both as fractions of `maximum` over the row's width. Runs on the
-- same occasions the bar value is set, so it eases along with the bar.
local function paintExtra(row, entry, rowWidth, maximum)
  local extra = entry.extra
  if not extra or extra <= 0 or maximum <= 0 then
    if row.lastExtraShown then
      row.lastExtraShown = false
      row.extra:Hide()
    end
    return
  end
  local start = min(rowWidth, rowWidth * (row.smoothValue or 0) / maximum)
  local width = min(rowWidth - start, rowWidth * extra / maximum)
  if width < 0.5 then
    if row.lastExtraShown then
      row.lastExtraShown = false
      row.extra:Hide()
  ...
  if row.lastExtraStart ~= start or row.lastExtraWidth ~= width then
    row.lastExtraStart, row.lastExtraWidth = start, width
    row.extra:SetPoint("TOPLEFT", row, "TOPLEFT", start, 0)
    row.extra:SetPoint("BOTTOMLEFT", row, "BOTTOMLEFT", start, 0)
    row.extra:SetWidth(width)
  end
  if not row.lastExtraShown then
    row.lastExtraShown = true
    row.extra:Show()
  end
end
...
function Renderer:CreateRow(index)
  ...
  -- The bar's dimmer continuation (a mode's extraField, overheal after
  -- effective healing): a texture on the row, so it sits under the bar frame
  -- and only its part past the fill's end shows. Positioned by arithmetic
  -- from the row's known width -- never from GetWidth, which this client
  -- reports in screen units for anchored frames -- and hidden when empty.
  row.extra = row:CreateTexture(nil, "ARTWORK")
  row.extra:SetTexture(Style:GetBarTexture())
  row.extra:SetPoint("TOPLEFT", row, "TOPLEFT", 0, 0)
  row.extra:SetPoint("BOTTOMLEFT", row, "BOTTOMLEFT", 0, 0)
  row.extra:SetWidth(1)
  row.extra:Hide()
...
function Renderer:PaintRows()
  ...
  local rowWidth = (profile.width or Style.MIN_WINDOW_WIDTH) - 2 * Style.WINDOW_PADDING
  ...
  for rowIndex = 1, paintRows do
    ...
    if entry then
      ...
      if row.lastBarTexture ~= barTexture or row.lastOpacity ~= windowOpacity then
        ...
        row.extra:SetTexture(barTexture)
      ...
      paintExtra(row, entry, rowWidth, maximum)
      ...
      if red ~= row.lastR or green ~= row.lastG or blue ~= row.lastB then
        ...
        row.extra:SetVertexColor(red, green, blue)
      end
      if row.lastAlpha ~= barAlpha then
        row.lastAlpha = barAlpha
        row.bar:SetAlpha(barAlpha)
        row.extra:SetAlpha(barAlpha * EXTRA_ALPHA)
      end
...
function Renderer:Animate()
  ...
  local rowWidth = self.rowWidth or ((self.db.width or Style.MIN_WINDOW_WIDTH) - 2 * Style.WINDOW_PADDING)
  local maximum = self.paintMaximum or 1
